...
static bool Main_FileExistsA(const std::string& fileName)
{
	if (fileName.empty())
		return false;
...
static bool Main_FilePackPairExistsA(const std::string& packBaseName)
{
	if (packBaseName.empty())
		return false;

	std::string hashName = packBaseName + ".hf";
	std::string packName = packBaseName + ".pf";

	return Main_FileExistsA(hashName) && Main_FileExistsA(packName);
}

bool GetUsingFilePack()
{
	// Este client depende do Pack01/Pack03 para fontes, NIFs, sprites e tabelas.
	// O comportamento antigo em Debug lia dmo.ini [DEBUG] FilePack.
	// Quando esse valor vinha 0 ou faltava, o client arrancava em modo disco/bin,
	// mas os recursos reais estão dentro dos packs. Isso causava os popups de:
	// Data\EtcObject\*.nif e Data\Bin\English\Digimon_List.bin.
	std::string resourcePack = GLOBALDATA_ST.GetLoadResourceFile();
	std::string tablePack = GLOBALDATA_ST.GetLoadTableFile();

	if (resourcePack.empty())
		resourcePack = "Data\\Pack01";

	if (tablePack.empty())
		tablePack = "Data\\Pack03";

	if (Main_FilePackPairExistsA(resourcePack) && Main_FilePackPairExistsA(tablePack))
	{
		OutputDebugStringA("[Main] FilePack enabled: pack files found.\\n");
		return true;
	}

	OutputDebugStringA("[Main] FilePack forced ON, but one or more pack files were not found.\\n");
	OutputDebugStringA("[Main] Resource pack: ");
	OutputDebugStringA(resourcePack.c_str());
	OutputDebugStringA("\\n");
	OutputDebugStringA("[Main] Table pack: ");
	OutputDebugStringA(tablePack.c_str());
	OutputDebugStringA("\\n");

	// Mesmo se o check físico falhar por working directory diferente, forçamos ON.
	// Se o pack realmente faltar, CsFPSystem::Initialize vai mostrar o erro correto.
	return true;
}
	...
	int       p_iCmdShow)
{
	...
	std::string resourcePackName = GLOBALDATA_ST.GetLoadResourceFile();
	std::string tablePackName = GLOBALDATA_ST.GetLoadTableFile();

	if (resourcePackName.empty())
		resourcePackName = "Data\\Pack01";

	if (tablePackName.empty())
		tablePackName = "Data\\Pack03";

	vecReadFiles.push_back(resourcePackName);

	if (tablePackName != resourcePackName)
		vecReadFiles.push_back(tablePackName);

	OutputDebugStringA("[Main] CsFPSystem initialize with packs:\\n");
	OutputDebugStringA("[Main]  - ");
	OutputDebugStringA(resourcePackName.c_str());
	OutputDebugStringA("\\n");
	OutputDebugStringA("[Main]  - ");
	OutputDebugStringA(tablePackName.c_str());
	OutputDebugStringA("\\n");

	if (!CsFPS::CsFPSystem::Initialize(true, vecReadFiles, false))
	{
#ifdef SPLASH_LOGO
		splash.CloseSplash();
#endif
