...
//////////////////////////////////////////////////////////////////////////
// DEBUG SECOND PASSWORD RECEIVE
//////////////////////////////////////////////////////////////////////////

static void DebugSecondPassRecvA(const char* message)
{
    OutputDebugStringA("[2PASS][RECV] ");
    OutputDebugStringA(message);
    OutputDebugStringA("\n");
}

static void DebugSecondPassRecvIntA(const char* label, int value)
{
    char buffer[256] = { 0, };
...
void cCliAccount::RecvCheck2ndPass(void)
{
    ...
    DebugSecondPassRecvA("RecvCheck2ndPass called");

    GS2C_RECV_2ndPass_Certify recv;

    pop(recv.nResult);

    DebugSecondPassRecvIntA("RecvCheck2ndPass nResult", recv.nResult);
...
void cCliAccount::Recv2ndPassRegister(void)
{
    ...
    DebugSecondPassRecvA("Recv2ndPassRegister called");

    GS2C_RECV_2ndPass_Register recv;

    pop(recv.nResult);

    DebugSecondPassRecvIntA("Recv2ndPassRegister nResult", recv.nResult);
...
void cCliAccount::Recv2ndPassChange(void)
{
    ...
    DebugSecondPassRecvA("Recv2ndPassChange called");

    GS2C_RECV_2ndPass_Change recv;

    pop(recv.nResult);

    DebugSecondPassRecvIntA("Recv2ndPassChange nResult", recv.nResult);
...
void cCliAccount::RecvClusterList(void)
{
    ...
    for (uint i = 0; i < cnt; i++)
    {
        ...
        pop(addInfo.m_nDisable);
        pop(addInfo.m_nClusterState);
