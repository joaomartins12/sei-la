...
{
    m_KeyPadWindow.InitScript(
        "Lobby\\SecondPassword\\lobby_popup_password.tga",
        CsPoint((1024 - 452) / 2, (768 - 460) / 2),
        CsPoint(452, 460),
        false,
        IFREGION_X::JUST_CENTER,
        IFREGION_Y::JUST_CENTER);

    m_KeyPadWindow.AddTitle(
        UISTRING_TEXT("SECONDPASSWORD_TITLE_SETUP").c_str(),
        CsPoint(0, 20),
        CFont::FS_16,
        NiColor(0.65f, 1.0f, 0.0f));

    m_KeyPadWindow.AddSprite(
        CsPoint(47, 75),
        CsPoint(358, 48),
        "Lobby\\SecondPassword\\lobby_Password_glow.tga");
    ...
    m_pOk = m_KeyPadWindow.AddButton(
        CsPoint(227, 404),
        CsPoint(137, 46),
        CsPoint(0, 46),
        "CommonUI\\Simple_btn_Ok.tga",
        cWindow::SD_Bu3);
    ...
    m_pCancel = m_KeyPadWindow.AddButton(
        CsPoint(90, 404),
        CsPoint(137, 46),
        CsPoint(0, 46),
        "CommonUI\\Simple_btn_Cancel.tga",
        cWindow::SD_Bu3);
    ...
    m_KeyPadWindow.SetVisible(true);
...
void cSecondPassword::_CreateWindow_RegistConfirm()
{
    ...
    m_KeyPadWindow.AddTitle(
        UISTRING_TEXT("SECONDPASSWORD_TITLE_CONFIRMSETUP").c_str(),
        CsPoint(0, 20),
        CFont::FS_16,
        NiColor(0.65f, 1.0f, 0.0f));
...
{
    m_AskWindow.InitScript(
        "Lobby\\popup_Notification.tga",
        CsPoint((1024 - 482) / 2, (768 - 288) / 2),
        CsPoint(482, 288),
        false,
        IFREGION_X::JUST_CENTER,
        IFREGION_Y::JUST_CENTER);

    m_AskWindow.AddTitle(
        UISTRING_TEXT("SECONDPASSWORD_REGIST_ASK_TITLE").c_str(),
        CsPoint(0, 20),
        CFont::FS_16,
        NiColor(1.0f, 0.17f, 0.17f));
    ...
    m_pCheckUseSPW = m_AskWindow.AddCheckBox(
        CsPoint(420, 195),
        CsPoint(19, 19),
        CsPoint(0, 19),
        "CommonUI\\CheckBT.tga",
        cWindow::SD_Wi1);
    ...
    m_pUsingSPW = m_AskWindow.AddButton(
        CsPoint(482 / 2 - 150, 230),
        CsPoint(137, 46),
        CsPoint(0, 46),
        "CommonUI\\Simple_btn_S.tga",
        cWindow::SD_Bu3);
    ...
    if (m_pUsingSPW)
    {
        ...
        m_pUsingSPW->AddEvent(cButton::BUTTON_LBUP_EVENT, this, &cSecondPassword::PressUsingButton);
    }

    m_pUsingLaterSPW = m_AskWindow.AddButton(
        CsPoint(482 / 2 + 13, 230),
        CsPoint(137, 46),
        CsPoint(0, 46),
        "CommonUI\\Simple_btn_S.tga",
        cWindow::SD_Bu3);
    ...
    if (m_pUsingLaterSPW)
    {
        ...
        m_pUsingLaterSPW->AddEvent(cButton::BUTTON_LBUP_EVENT, this, &cSecondPassword::PressLaterButton);
    }

    m_AskWindow.SetVisible(true);
    m_AskWindow.SetEnableWindow(true);

    /*
     * Enquanto a ask window está visível, bloqueia a keypad window.
     * Isto evita cliques por trás da popup.
     */
    m_KeyPadWindow.SetEnableWindow(false);
}

void cSecondPassword::PressUsingButton(void* pSender, void* pData)
{
    m_AskWindow.SetVisible(false);
    m_AskWindow.SetEnableWindow(false);
...
{
    /*
     * Evita duplo clique enquanto a janela seguinte é recriada.
     * cButton não tem SetEnableWindow(), por isso limpamos o ponteiro local.
     */
    m_pOk = NULL;
...
{
    /*
     * Evita envio duplo enquanto aguardamos resposta do server.
     * cButton não tem SetEnableWindow(), por isso limpamos o ponteiro local.
     */
    m_pOk = NULL;
