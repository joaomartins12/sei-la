...
namespace
{
	...
	static CsPoint GetServerListAbsolutePos(size_t nServerCount)
	{
		const int listW = GetServerListWidth(nServerCount);

		return CsPoint(
			(g_nScreenWidth - listW) / 2,
			((g_nScreenHeight - 410) / 2) + SERVER_LIST_Y_IN_WINDOW
		);
	}
}
...
void CSelectServer::_MakeListControl(size_t const& nServerCount)
{
	...
	CsPoint uiSize(SERVER_CARD_W, SERVER_CARD_H);
	uiSize.x = GetServerListWidth(nServerCount);

	// cGridListBox não herda corretamente o offset do m_SelectListWindow nesta tela.
	// Por isso usamos posição absoluta responsiva, centrada no ecrã.
	m_pServerListBox->Init(
		GetRoot(),
		GetServerListAbsolutePos(nServerCount),
		uiSize,
		CsPoint(5, 0),
		CsPoint(154, 314),
...
void CSelectServer::_ChangeListSize(size_t const& nServerCount)
{
	...
	m_pServerListBox->SetPos(GetServerListAbsolutePos(nServerCount));
