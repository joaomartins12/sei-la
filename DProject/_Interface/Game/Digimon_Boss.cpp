...
namespace
{
	static std::string Boss_ToSlashPathA(const std::string& path)
	{
	...
	static std::string Boss_ToBackSlashPathA(const std::string& path)
	{
		std::string result = path;
	...
	static std::string Boss_UpperSystemA(const std::string& path)
	{
		std::string result = path;
	...
	static bool Boss_PathExistsInPackA(const std::string& fullPath)
	{
		if (fullPath.empty())
			return false;

		if (CsFPS::CsFPSystem::IsExistOnlyPack(0, fullPath.c_str()))
			return true;

		if (CsFPS::CsFPSystem::IsExistOnlyPack(1, fullPath.c_str()))
			return true;

		return false;
	}

	static bool Boss_EffectExistsA(const std::string& relativePath)
	{
		if (relativePath.empty())
			return false;

		std::string relSlash = Boss_ToSlashPathA(relativePath);
		std::string relBackSlash = Boss_ToBackSlashPathA(relativePath);

		std::string candidates[] =
		{
			"Data\\Effect\\" + relBackSlash,
			"data\\Effect\\" + relBackSlash,
			"Data\\effect\\" + relBackSlash,
			"data\\effect\\" + relBackSlash,

			"Data/Effect/" + relSlash,
			"data/Effect/" + relSlash,
			"Data/effect/" + relSlash,
			"data/effect/" + relSlash
		};
		...
		{
			if (Boss_PathExistsInPackA(candidates[i]))
				return true;
		}
	...
	static std::string Boss_ResolveEffectPathA(const char* requestedRelativePath)
	{
		if (requestedRelativePath == NULL || requestedRelativePath[0] == '\0')
			return "";

		std::string requested = requestedRelativePath;

		std::string candidates[] =
		{
			requested,
			Boss_ToSlashPathA(requested),
			Boss_ToBackSlashPathA(requested),
			Boss_UpperSystemA(requested),
			Boss_ToSlashPathA(Boss_UpperSystemA(requested)),
			Boss_ToBackSlashPathA(Boss_UpperSystemA(requested)),

			// Fallbacks vistos nos popups do client.
			"System\\Digimon_Gate.nif",
			"System/Digimon_Gate.nif",
			"system\\Digimon_Gate.nif",
			"system/Digimon_Gate.nif",

			"System\\Boss_Digimon_gate.nif",
			"System/Boss_Digimon_gate.nif",
			"system\\Boss_Digimon_gate.nif",
			"system/Boss_Digimon_gate.nif"
		};
		...
		for (int i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i)
		{
			...
			if (Boss_EffectExistsA(candidates[i]))
			{
				OutputDebugStringA("[BossScene] Resolved effect path: ");
				OutputDebugStringA(candidates[i].c_str());
				OutputDebugStringA("\n");
		...
		OutputDebugStringA("[BossScene] Effect path not found in Pack01/Pack03, using requested path: ");
		OutputDebugStringA(requested.c_str());
		OutputDebugStringA("\n");
...
void cBossScene::SetEmergency(DWORD dwMonType, int nCount)
{
	...
	std::string strGateEffect = Boss_ResolveEffectPathA("system\\Boss_Digimon_gate.nif");
	std::string strFinalityEffect = Boss_ResolveEffectPathA("system\\LevelEffect\\BossFinalityEffect.nif");

	m_pBossMon->GetProp_Effect()->AddEffect((char*)strGateEffect.c_str(), fScale, dwFlag);
	m_pBossMon->GetProp_Effect()->AddLoopEffect(CsC_EffectProp::LE_BOSS, (char*)strFinalityEffect.c_str());
}
