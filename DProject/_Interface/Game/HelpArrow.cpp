...
namespace
{
	static std::string HA_ToSlashPathA(const char* path)
	{
		std::string result;
	...
	static std::string HA_ToBackSlashPathA(const char* path)
	{
		std::string result;
	...
	static std::string HA_LowerFirstDataA(const std::string& path)
	{
	...
	static std::string HA_UpperFirstDataA(const std::string& path)
	{
		std::string result = path;
	...
	static bool HA_TryLoadNiStream(NiStream& stream, const std::string& path)
	{
		if (path.empty())
			return false;
	...
	static bool HA_LoadNiStreamSafe(NiStream& stream, const char* originalPath, std::string& loadedPath)
	{
		loadedPath.clear();
		...
		candidates[0] = originalPath;
		candidates[1] = HA_ToSlashPathA(originalPath);
		candidates[2] = HA_ToBackSlashPathA(originalPath);
		candidates[3] = HA_LowerFirstDataA(candidates[0]);
		candidates[4] = HA_LowerFirstDataA(candidates[1]);
		candidates[5] = HA_LowerFirstDataA(candidates[2]);
		candidates[6] = HA_UpperFirstDataA(candidates[1]);
		candidates[7] = HA_UpperFirstDataA(candidates[2]);
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			if (HA_TryLoadNiStream(stream, candidates[i]))
			{
				loadedPath = candidates[i];

				OutputDebugStringA("[HelpArrow] Loaded QuestArrow from: ");
		...
		OutputDebugStringA("[HelpArrow] Failed to load QuestArrow. Original path: ");
...
#define ARROWPOINT_OBJECT_PATH		"Data\\EtcObject\\QuestArrow.nif"
	...
	if (!HA_LoadNiStreamSafe(kStream, ARROWPOINT_OBJECT_PATH, loadedPath))
	{
	...
	if (pNode == NULL)
	{
		CsMessageBoxA(MB_OK, "%s Loaded, but object is NULL", loadedPath.c_str());
		return;
	}
