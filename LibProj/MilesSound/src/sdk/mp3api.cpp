...
enum PROPERTY
{
   ...
   SPU_FLAGS=0x40000000,   // Return the flags for spu usage.
   PROF_INDEX=0x1002       // profile index: 0x1001=bink,0x1002=mp3,0x1003=ogg, 
};
...
S32 fetch_audio_data(ASISTREAM *STR)
{
   ...
   // If not, leave 768 bytes on the front (which is conservative for the 
   // bit res backup amount)
   //

   if (STR->write_cursor + STR->data_size >= (STREAM_BUFSIZE-4))
   {
      U32 keep_pos = STR->write_cursor - 768;
      
      AIL_memcpy(STR->audio_buffer,
                &STR->audio_buffer[ keep_pos ],
                 STR->write_cursor - keep_pos );
...
//       strcpy(ASI_error_text,"End of data reached");
   ...
   *((U32 *)(void*)STR->header_buffer) = seek_data;
...
static S32 AILCALL PROVIDER_property(HPROPERTY index, void * before_value, void const * new_value, void * after_value)
{
   switch (index)
      ...
      case PROVIDER_NAME:
        if (before_value)
        {
          *(char const **)before_value = "MSS MPEG Layer 3 Audio Decoder";
          return 1;
        }
        break;
      ...
      case IN_ED_FTYPES:
        if (before_value)
        {
          *(char const **)before_value = "MPEG Layer 3 audio files\0*.MP3\0";
          return 1;
        }
        break;
      ...
      case FRAME_SIZE:
        if (before_value)
        {
          *(S32*)before_value = (STREAM_BUFSIZE / 2);
          return 1;
        }
        break;
      }
                                           ...
                                           U32           total_size)
{
   ...
   if ( !L3_init( palloc, pfree ) )
     goto no_mem;
 
   STR = (ASISTREAM *) palloc((sizeof(ASISTREAM) + 15) & ~15,user,__FILE__,__LINE__);
   ...
   STR->frame_buffer = (U8 *) palloc(FRAME_BUFSIZE,user,__FILE__,__LINE__);

   if (STR->frame_buffer == NULL)
      {
      strcpy(ASI_error_text,"Out of memory");
      pfree(STR,user,__FILE__,__LINE__);
      return 0;
      }
...
static ASIRESULT      AILCALL ASI_stream_close(HASISTREAM stream)
{
   ...
   if (STR->frame_buffer != NULL)
      {
      STR->free_CB(STR->frame_buffer,STR->user,__FILE__,__LINE__);
                                         ...
                                         S32       request_size)
{
   ...
   if ( request_size == -1 )
   {
     if ( ( FRAME_BUFSIZE - STR->frame_size ) < 2400 )  // conservative room for one mp3 frame
     {
       if ( ( STR->frame_size - STR->output_cursor ) < 3600 )
       {
         S32 adj_pos = STR->output_cursor & ~15;
         // halfway through eating up the output buffer, 
         AIL_memcpy( STR->frame_buffer, STR->frame_buffer + adj_pos, STR->frame_size - adj_pos );
         STR->frame_size -= adj_pos;
         STR->samples -= adj_pos;
         STR->output_cursor -= adj_pos;
       }
       else
       {
         // no room
     ...
     do
     {
       ...
       check_pos = 8 * (incoming - STR->main_data_begin);
     } while ( check_pos < 0 );
                                            ...
                                            void * after_value)
{
   ...
   switch (property)
      ...
      case INPUT_SAMPLE_RATE:
        if (before_value) 
        {
          *(S32*)before_value = sample_rate[STR->MPEG25][STR->MPEG1][STR->sampling_frequency];
          return 1;
        }
        break;
      ...
      case INPUT_BITS:
        if (before_value) 
        {
          *(S32*)before_value = 16 / max(1,((sample_rate[STR->MPEG25][STR->MPEG1][STR->sampling_frequency] * 16 * STR->nch) / STR->bit_rate));
          return 1;
        }
        break;
      ...
      case OUTPUT_BIT_RATE:   
        if (before_value) 
        {
          *(S32*)before_value = sample_rate[STR->MPEG25][STR->MPEG1][STR->sampling_frequency] * 16 * STR->nch;
          return 1;
        }
        break;
      ...
      case OUTPUT_RESERVOIR: 
        if (before_value) 
        {
          *(S32*)before_value = STR->frame_size - STR->output_cursor;
          return 1;
        }
        break;
      ...
      case EMITTED_SAMPLE_COUNT:
        if (before_value) 
        {
          *(S64 *) before_value = STR->emitted_sample_count;
          return 1;
        }
        break;
#endif
      ...
      case PERCENT_DONE:
        if (before_value) 
        {
         ...
         {
            *(F32*)before_value = F32(STR->current_offset) * 100.0F / F32(STR->total_size);
         }
         return 1;
        }
        break;
...
DXDEF S32 AILEXPORT RIB_MAIN_NAME(MP3Dec)( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
{
    ...
    const RIB_INTERFACE_ENTRY ASI4[] =
      {
      REG_PR("Profile index",                         PROF_INDEX,          (RIB_DATA_SUBTYPE) (RIB_READONLY|RIB_DEC)),
      REG_PR("SPU flags",                             SPU_FLAGS,           (RIB_DATA_SUBTYPE) (RIB_READONLY|RIB_DEC)),
      };
    ...
    const RIB_INTERFACE_ENTRY ASISTR6[] =
     ...
     if (up_down)
     {
         ...
         RIB_registerP(provider_handle, "ASI codec", ASI4);
...
RADEXPFUNC void * RADEXPLINK miles_mp3_open(MEM_ALLOC_TYPE * palloc,
                                MEM_FREE_TYPE  * pfree,
                                void *info, MILES_MP3_FETCH *getdata, S32 datalen)
{
   return (void*) ASI_stream_open((MSS_ALLOC_TYPE*)palloc, (MSS_FREE_TYPE*)pfree, (UINTa) info, (AILASIFETCHCB)getdata, datalen);
}

RADEXPFUNC S32 RADEXPLINK miles_mp3_getdata(void *ptr, S16 *samples, S32 num_samples, mp3_info *info)
{
   S32 bytes;
   ASISTREAM *str = (ASISTREAM *) ptr;
   bytes = ASI_stream_process((HASISTREAM) ptr, samples, num_samples);
   if (info) {
      info->rate = sample_rate[str->MPEG25][str->MPEG1][str->sampling_frequency];
      info->channels = str->nch;
   }
   return bytes;
}

RADEXPFUNC void RADEXPLINK miles_mp3_close(void *info)
{
   ASI_stream_close((HASISTREAM) info);
}

// Iggy mp3 interface

struct IggyMP3Interface iggy_mp3_interface;

static void * RADLINK iggy_miles_alloc(UINTa size, UINTa user, const char *file, U32 line)
{
   RR_UNUSED_VARIABLE(file);
   RR_UNUSED_VARIABLE(line);
   return iggy_mp3_interface.iggy_malloc(user, (S32) size);
}

static void RADLINK iggy_miles_free(void *p, UINTa user, const char *file, U32 line)
{
   RR_UNUSED_VARIABLE(file);
   RR_UNUSED_VARIABLE(line);
   iggy_mp3_interface.iggy_free(user ,p);
}

static void * RADLINK iggy_miles_mp3_open(void *info, MILES_MP3_FETCH *getdata, S32 datalen)
{
   return miles_mp3_open(iggy_miles_alloc, iggy_miles_free, info, getdata, datalen);
}

static void RADLINK iggy_miles_mp3_close(void *info)
{
   return miles_mp3_close(info);
}

static S32 RADLINK iggy_miles_mp3_getdata(void *ptr, S16 *samples, S32 num_samples, mp3_info *info)
{
   return miles_mp3_getdata(ptr, samples, num_samples, info);
}

static rrbool IggyAudioInitStruct(struct IggyMP3Interface *interface)
{
   if (sizeof(iggy_mp3_interface) != interface->struct_size)
      return 0;

   iggy_mp3_interface = *interface;
   interface->open = iggy_miles_mp3_open;
   interface->close = iggy_miles_mp3_close;
   interface->getdata = iggy_miles_mp3_getdata;
