...
namespace
{
	static std::string DGM_ToSlashPathA(const std::string& path)
	{
	...
	static std::string DGM_ToBackSlashPathA(const std::string& path)
	{
		std::string result = path;
	...
	static std::string DGM_NormalizeCompareA(const std::string& path)
	{
		std::string result = DGM_ToBackSlashPathA(path);
	...
	static std::string DGM_GetFileNameOnlyA(const std::string& path)
	{
	...
	static bool DGM_EndsWithA(const std::string& text, const std::string& suffix)
	{
	...
	static bool DGM_FileExistsA(const std::string& path)
	{
		if (path.empty())
			return false;
	...
	static void DGM_CreateFolderRecursiveA(const std::string& folder)
	{
		if (folder.empty())
			return;
	...
	static void DGM_CreateDirectoryForFileA(const std::string& filePath)
	{
		...
		DGM_CreateFolderRecursiveA(filePath.substr(0, pos));
	}

	static bool DGM_FindPackPathA(const std::string& requestedPath, std::string& resolvedPath)
	{
		resolvedPath.clear();

		std::string reqBack = DGM_ToBackSlashPathA(requestedPath);
		std::string reqSlash = DGM_ToSlashPathA(requestedPath);
		...
		for (int packIndex = 0; packIndex < 2; ++packIndex)
		{
			...
			{
				if (CsFPS::CsFPSystem::IsExistOnlyPack(packIndex, candidates[i].c_str()))
				{
		...
		std::string reqCompare = DGM_NormalizeCompareA(requestedPath);
		std::string reqFile = DGM_NormalizeCompareA(DGM_GetFileNameOnlyA(requestedPath));
		...
		for (int packIndex = 0; packIndex < 2; ++packIndex)
		{
			...
			for (std::list<std::string>::iterator it = files.begin(); it != files.end(); ++it)
			{
				...
				std::string packCompare = DGM_NormalizeCompareA(packPath);
			...
			for (std::list<std::string>::iterator it = files.begin(); it != files.end(); ++it)
			{
				...
				if (DGM_EndsWithA(packCompare, reqCompare))
				{
					resolvedPath = packPath;
					return true;
				}
			...
			for (std::list<std::string>::iterator it = files.begin(); it != files.end(); ++it)
			{
				...
				if (DGM_EndsWithA(packCompare, "\\" + reqFile) &&
					packCompare.find("\\bin\\") != std::string::npos)
				{
					resolvedPath = packPath;
					return true;
				}
	...
	static bool DGM_ExtractPackFileToDriveA(const std::string& drivePath)
	{
		if (drivePath.empty())
			return false;

		if (DGM_FileExistsA(drivePath))
			return true;

		std::string packPath;
		if (!DGM_FindPackPathA(drivePath, packPath))
		{
			OutputDebugStringA("[DigimonMng] BIN not found in pack: ");
			OutputDebugStringA(drivePath.c_str());
			OutputDebugStringA("\n");
			return false;
		}
		...
		char* pData = NULL;
		size_t dataSize = 0;
		int foundPackIndex = -1;

		for (int packIndex = 0; packIndex < 2; ++packIndex)
		{
			dataSize = CsFPS::CsFPSystem::Allocate_GetFileData(packIndex, &pData, packPath.c_str());
		...
		if (pData == NULL || dataSize == 0)
		{
			OutputDebugStringA("[DigimonMng] Allocate_GetFileData failed: ");
			OutputDebugStringA(packPath.c_str());
			OutputDebugStringA("\n");
			return false;
		}

		DGM_CreateDirectoryForFileA(drivePath);

		FILE* fp = NULL;
		fopen_s(&fp, drivePath.c_str(), "wb");
		...
		OutputDebugStringA("[DigimonMng] Extracted BIN from pack to drive: ");
		OutputDebugStringA(drivePath.c_str());
		OutputDebugStringA(" <= ");
		OutputDebugStringA(packPath.c_str());
		OutputDebugStringA("\n");
	...
	static bool DGM_EnsureBinFileA(const char* cPath, const char* binName)
	{
		if (cPath == NULL || binName == NULL)
			return false;

		char cName[MAX_PATH] = { 0 };
		sprintf_s(cName, MAX_PATH, "%s\\%s", cPath, binName);

		return DGM_ExtractPackFileToDriveA(cName);
	}

	static void DGM_EnsureAllDigimonBinFilesA(const char* cPath)
	{
		DGM_EnsureBinFileA(cPath, DIGIMON_TABLE_BIN);
#ifdef CROSSWARS_SYSTEM
		DGM_EnsureBinFileA(cPath, CROSSDIGIMON_TABLE_BIN);
#endif
		DGM_EnsureBinFileA(cPath, RIDE_TABLE_BIN);
		DGM_EnsureBinFileA(cPath, NEW_ELEMENT_TABLE_BIN);
		DGM_EnsureBinFileA(cPath, ENCYCLOPEDIA_BIN);
		DGM_EnsureBinFileA(cPath, PASSIVE_ABILITY_BIN);
		DGM_EnsureBinFileA(cPath, DATA_EXCHAGE_SHEET_BIN);
	}
}
...
{
	// Em modo FT_BIN o código antigo tentava abrir Data\Bin\... diretamente do disco.
	// Como neste client os .bin estão dentro do Pack01/Pack03, garantimos primeiro
	// que estes ficheiros existem como cache física para o loader antigo conseguir ler.
	DGM_EnsureAllDigimonBinFilesA(cPath);
